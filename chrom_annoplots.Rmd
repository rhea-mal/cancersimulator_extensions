...
copynumber <- createCNmatrix(CNbins_PCAWG, field = 'state', wholegenome = TRUE,
...
```{r}
## convert joined (PCAWG) into CN bins format
CNbins_PCAWG <- joined[-c(9:19)]
names(CNbins_PCAWG)[names(CNbins_PCAWG) == 'chromosome'] <- 'chr'
names(CNbins_PCAWG)[names(CNbins_PCAWG) == 'total_cn'] <- 'copy'
CNbins_PCAWG$major_cn = CNbins_PCAWG$copy
names(CNbins_PCAWG)[names(CNbins_PCAWG) == 'major_cn'] <- 'state'
names(CNbins_PCAWG)[names(CNbins_PCAWG) == 'aliquot_id'] <- 'cell_id'
CNbins_PCAWG <- CNbins_PCAWG[-c(6:7)]
```
